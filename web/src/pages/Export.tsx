...
export function ExportPage() {
  ...
  useEffect(() => {
    if (!dossierId) return;
    apiClient.importsGa.list(dossierId).then(setExports).catch(() => {});
  }, [dossierId]);
  ...
  const generate = async () => {
    ...
    setMsg('');
    try {
      const res = await apiClient.importsGa.generate(dossierId);
      setMsg(`Fichiers générés : ${res.exports.map((e: any) => `${e.type} (${e.nb_lignes} lignes)`).join(', ')}`);
      apiClient.importsGa.list(dossierId).then(setExports).catch(() => {});
    } catch (e: any) {
      setMsg(e.message);
    }
  ...
  const download = async (exportId: string, filename: string) => {
    try {
      const blob = await apiClient.importsGa.download(exportId);
  ...
  return (
    <div className="space-y-4 mt-4">
      ...
      <div className="bg-white border rounded-lg p-4 flex gap-3 items-end">
        ...
        {dossierId && (
          <button onClick={generate} disabled={generating}
            ...
            {generating ? 'Génération...' : 'Générer fichiers Sage GA'}
          </button>
        )}
      </div>
              ...
              <div key={exp.id} className="flex items-center justify-between border-b pb-2 last:border-b-0">
                <div>
                  <span className="text-sm font-medium">
                    {exp.type_import === 'salaries' ? 'Import Salariés' : 'Import Variables'}
                  </span>
                  <span className="text-xs text-gray-400 ml-2">{exp.nb_lignes} lignes</span>
                </div>
                <button onClick={() => download(exp.id, exp.fichier_r2_key.split('/').pop())}
      ...
      {dossierId && exports.length === 0 && (
        <p className="text-sm text-gray-400">Aucun fichier exporté pour ce dossier</p>
      )}
    </div>
  );
}
